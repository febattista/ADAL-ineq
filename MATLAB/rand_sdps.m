 function [A, b, C] = rand_sdps(n, m, mleq, p, rand_seed);
...
A = sparse(n*n,m);
for j = 1:m
% if mod(j,1000)==0; fprintf(' j= %6.0d \n', j); end
...
% y(1:mleq) must be positive
% then b = A(X), and C = A'(y) + Z

y = randn(m,1);
y(1:mleq) = - abs(y(1:mleq));
% A(:, 1:mleq) = -A(:, 1:mleq);
b = A*reshape(X, n*n, 1) + [x; zeros(m-mleq, 1)] ;
C = reshape(A'*y, n, n) + Z;      
